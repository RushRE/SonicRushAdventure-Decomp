...
SRC_SUBDIR                := src
ASM_SUBDIR                := asm
LIB_SRC_SUBDIR            := lib/src $(LIB_SUBDIRS:%=$(ROOT_DIR)lib/%/src) $(NITROSDK_SRC_SUBDIRS:%=$(ROOT_DIR)lib/NitroSDK/src/%)
LIB_ASM_SUBDIR            := lib/asm/$(BUILD_MODE) $(LIB_SUBDIRS:%=$(ROOT_DIR)lib/%/asm/$(BUILD_MODE))
...
# NitroSDK was compiled with an older version and then linked to the game, so replicate that!
