...
BOOL CTRDGi_IsNumberIDAtInit(u8 numberID)
{
    ...
    if (cip->moduleID.raw != 0xffff && cip->moduleID.raw != 0x0000)
    {
        ...
        {
            if (cip->exLsiID[0] == numberID || cip->exLsiID[1] == numberID || cip->exLsiID[2] == numberID)
            {
                retval = TRUE;
            }
        }
...
BOOL CTRDG_IsExisting(void)
{
...

    {
        ...
        if ((isRomCode == CTRDG_IS_ROM_CODE && cip->moduleID.raw != chp->moduleID) || (isRomCode != CTRDG_IS_ROM_CODE && cip->moduleID.raw != *CTRDGi_GetModuleIDImageAddr())
...
#endif
{
    while (1)
    {
        ...
        if (((info->locked = OS_ReadOwnerOfLockCartridge() & CTRDG_LOCKED_BY_MYPROC_FLAG) != 0) || (OS_TryLockCartridge(lockID) == OS_LOCK_SUCCESS))
        {
#ifdef SDK_ARM7
            return TRUE;
#else
