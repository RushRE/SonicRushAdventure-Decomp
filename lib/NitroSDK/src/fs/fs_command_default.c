...
FSResult (*const(fsi_default_command[]))(FSFile *) = {
    FSi_ReadFileCommand, FSi_WriteFileCommand,    FSi_SeekDirCommand,        FSi_ReadDirCommand,   FSi_FindPathCommand,
    FSi_GetPathCommand,  FSi_OpenFileFastCommand, FSi_OpenFileDirectCommand, FSi_CloseFileCommand,
};
...
static FSResult FSi_FindPathCommand(FSFile *p_dir)
{
    ...
    for (; MI_ReadByte(path); path += (MI_ReadByte(path) ? 1 : 0))
    {
        ...
        while ((is_directory = MI_ReadByte(path + name_len)), (is_directory && !FSi_IsSlash(is_directory)))
            ++name_len;
        ...
        else
        {
            ...
            for (;;)
            {
                ...
                if ((is_directory != etr.is_directory) || (name_len != etr.name_len) || FSi_StrNICmp(path, etr.name, (u32)name_len))
                    continue;
