...
static const u8 fsi_def_digest_key[64] = {
    0x21, 0x06, 0xc0, 0xde, 0xba, 0x98, 0xce, 0x3f, 0xa6, 0x92, 0xe3, 0x9d, 0x46, 0xf2, 0xed, 0x01, 0x76, 0xe3, 0xcc, 0x08, 0x56, 0x23,
    0x63, 0xfa, 0xca, 0xd4, 0xec, 0xdf, 0x9a, 0x62, 0x78, 0x34, 0x8f, 0x6d, 0x63, 0x3c, 0xfe, 0x22, 0xca, 0x92, 0x20, 0x88, 0x97, 0x23,
    0xd2, 0xcf, 0xae, 0xc2, 0x32, 0x67, 0x8d, 0xfe, 0xca, 0x83, 0x64, 0x98, 0xac, 0xfd, 0x3e, 0x37, 0x87, 0x46, 0x58, 0x24,
};
...
{
    u32 size = (((p_ovi->header.flag & FS_OVERLAY_FLAG_COMP) != 0) ? p_ovi->header.compressed : p_ovi->header.ram_size);
    return size;
}
    ...
    for (;;)
    {
...

        {
            ...
            while (p)
            {
                ...
                if (((obj == 0) && (dtor >= region_top) && (dtor < region_bottom)) || ((obj >= region_top) && (obj < region_bottom)))
                {
...
BOOL FS_LoadOverlay(MIProcessor target, FSOverlayID id)
{
    const u8 *const base = (const u8 *)(((OS_GetConsoleType() & OS_CONSOLE_DEV_CARD) != 0) ? 0x00000000 : HW_CTRDG_ROM);
    ...
    FSi_ReadRomDirect(base + fat_info.top, ovi.header.ram_address, fat_info.bottom - fat_info.top);
