...
#endif
    const void *
    ConvAddr(const void *p, const void *baseFrom, const void *baseTo)
{
    if (p == NULL)
        return NULL;
...
{
#ifdef SDK_ARM9
    DC_InvalidateRange((void *)&SNDi_SharedWork->playerStatus, sizeof(SNDi_SharedWork->playerStatus));
#endif
...
{
#ifdef SDK_ARM9
    DC_InvalidateRange((void *)&SNDi_SharedWork->channelStatus, sizeof(SNDi_SharedWork->channelStatus));
#endif
...
{
#ifdef SDK_ARM9
    DC_InvalidateRange((void *)&SNDi_SharedWork->captureStatus, sizeof(SNDi_SharedWork->captureStatus));
#endif
...
{
#ifdef SDK_ARM9
    DC_InvalidateRange((void *)&SNDi_SharedWork->player[playerNo].tickCounter, sizeof(SNDi_SharedWork->player[playerNo].tickCounter));
#endif
...
{
#ifdef SDK_ARM9
    DC_InvalidateRange((void *)&SNDi_SharedWork->player[playerNo].variable[varNo], sizeof(SNDi_SharedWork->player[playerNo].variable[varNo]));
#endif
...
{
#ifdef SDK_ARM9
    DC_InvalidateRange((void *)&SNDi_SharedWork->globalVariable[varNo], sizeof(SNDi_SharedWork->globalVariable[varNo]));
#endif
...
BOOL SND_ReadChannelInfo(const SNDDriverInfo *driverInfo, int chNo, SNDChannelInfo *chInfo)
{
    ...
    if (chInfo->lockFlag)
    {
        ...
        {
            const u8 volume                 = (u8)((chCtrl & REG_SND_SOUND0CNT_VOLUME_MASK) >> REG_SND_SOUND0CNT_VOLUME_SHIFT);
            const SNDChannelDataShift shift = (SNDChannelDataShift)((chCtrl & REG_SND_SOUND0CNT_SHIFT_MASK) >> REG_SND_SOUND0CNT_SHIFT_SHIFT);
...
BOOL SND_ReadTrackInfo(const SNDDriverInfo *driverInfo, int playerNo, int trackNo, SNDTrackInfo *trackInfo)
{
    ...
    {
        const SNDExChannel *chp = (const SNDExChannel *)ConvAddr(track->channel_list, driverInfo->workAddress, &driverInfo->work);
        ...
        while (chp != NULL)
        {
            ...
            chp = (const SNDExChannel *)ConvAddr(chp->nextLink, driverInfo->workAddress, &driverInfo->work);
        }
    }
...
{
#ifdef SDK_ARM9
    DC_InvalidateRange((void *)&SNDi_SharedWork->finishCommandTag, sizeof(SNDi_SharedWork->finishCommandTag));
#endif
