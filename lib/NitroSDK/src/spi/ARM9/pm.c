...
u32 PMi_ReadRegister(u16 registerAddr, u16 *buffer)
{
    ...
    u32 sendResult = PMi_ReadRegisterAsync(registerAddr, buffer, PMi_DummyCallback, &commandResult);
    if (sendResult == PM_SUCCESS)
    {
        PMi_WaitBusy();
        return commandResult;
...
{
    u32 commandResult;
    u32 sendResult = PMi_WriteRegisterAsync(registerAddr, data, PMi_DummyCallback, &commandResult);
    if (sendResult == PM_SUCCESS)
    {
        PMi_WaitBusy();
        return commandResult;
...
u32 PM_GetBattery(PMBattery *batteryBuf)
{
    ...
    if ((result = PMi_ReadRegister(REG_PMIC_STAT_ADDR, &reg)) == PM_SUCCESS)
    {
        if (batteryBuf)
        {
            *batteryBuf = (PMBattery)(reg & PMi_STAT_BATTERY_MASK) ? PM_BATTERY_LOW : PM_BATTERY_HIGH;
        }
    }
...
void PM_GoSleepMode(PMWakeUpTrigger trigger, PMLogic logic, u16 keyPattern)
{
    ...
    {
        u16 param;
        param = (u16)(trigger | preTop << PM_BACKLIGHT_RECOVER_TOP_SHIFT | preBottom << PM_BACKLIGHT_RECOVER_BOTTOM_SHIFT);
