...
static NNS_G3D_INLINE void GetMatColAnm_(const NNSG3dResMatCAnm *pAnm, u16 idx, u32 frame, NNSG3dMatAnmResult *pResult)
{
    ...
    pResult->prmMatColor0 = (pResult->prmMatColor0 & REG_G3_DIF_AMB_C_MASK) | diffuse | (ambient << REG_G3_DIF_AMB_AMBIENT_RED_SHIFT);

    emission              = GetMatColAnmValue_(pAnm, pAnmData->emission, frame);
    specular              = GetMatColAnmValue_(pAnm, pAnmData->specular, frame);
    pResult->prmMatColor1 = (pResult->prmMatColor1 & REG_G3_SPE_EMI_S_MASK) | specular | (emission << REG_G3_SPE_EMI_EMISSION_RED_SHIFT);
