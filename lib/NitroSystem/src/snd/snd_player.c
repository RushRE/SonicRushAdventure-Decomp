...
// small fix for "NNS_SndHandleIsValid" not matching, for some reason?
#define NNSi_SndHandleIsValid(handle) ((handle)->player != NULL)
...
void NNS_SndPlayerStopSeqByPlayerNo(int playerNo, int fadeFrame)
{
    ...
    for (i = 0; i < SND_PLAYER_NUM; i++)
    {
        ...
        if (seqPlayer->status != NNS_SND_SEQ_PLAYER_STATUS_STOP && seqPlayer->player == &sPlayer[playerNo])
        {
...
void NNS_SndPlayerStopSeqBySeqNo(int seqNo, int fadeFrame)
{
    ...
    for (i = 0; i < SND_PLAYER_NUM; i++)
    {
        ...
        if (seqPlayer->status != NNS_SND_SEQ_PLAYER_STATUS_STOP && seqPlayer->seqType == NNS_SND_PLAYER_SEQ_TYPE_SEQ && seqPlayer->seqNo == seqNo)
        {
            NNSi_SndPlayerStopSeq(seqPlayer, fadeFrame);
        }
    }
...
void NNS_SndPlayerStopSeqBySeqArcNo(int seqArcNo, int fadeFrame)
{
    ...
    for (i = 0; i < SND_PLAYER_NUM; i++)
    {
        ...
        if (seqPlayer->status != NNS_SND_SEQ_PLAYER_STATUS_STOP && seqPlayer->seqType == NNS_SND_PLAYER_SEQ_TYPE_SEQARC && seqPlayer->seqNo == seqArcNo)
        {
            NNSi_SndPlayerStopSeq(seqPlayer, fadeFrame);
        }
    }
...
{
    if (!NNSi_SndHandleIsValid(handle))
        return;
...
int NNS_SndPlayerCountPlayingSeqBySeqNo(int seqNo)
{
    ...
    {
        if (seqPlayer->seqType == NNS_SND_PLAYER_SEQ_TYPE_SEQ && seqPlayer->seqNo == seqNo)
        {
            count++;
        }
    }
...
int NNS_SndPlayerCountPlayingSeqBySeqArcNo(int seqArcNo)
{
    ...
    while ((seqPlayer = (NNSSndSeqPlayer *)NNS_FndGetNextListObject(&sPrioList, seqPlayer)) != NULL)
    {
        if (seqPlayer->seqType == NNS_SND_PLAYER_SEQ_TYPE_SEQARC && seqPlayer->seqNo == seqArcNo)
        {
            count++;
        }
    }
...
int NNS_SndPlayerCountPlayingSeqBySeqArcIdx(int seqArcNo, int index)
{
    ...
    while ((seqPlayer = (NNSSndSeqPlayer *)NNS_FndGetNextListObject(&sPrioList, seqPlayer)) != NULL)
    {
        if (seqPlayer->seqType == NNS_SND_PLAYER_SEQ_TYPE_SEQARC && seqPlayer->seqNo == seqArcNo && seqPlayer->seqArcIndex == index)
        {
            count++;
        }
    }
...
void NNS_SndPlayerSetTrackVolume(NNSSndHandle *handle, u16 trackBitMask, int volume)
{
    ...
    SND_SetTrackVolume(handle->player->playerNo, trackBitMask, SND_CalcDecibel(volume));
}
...
void NNSi_SndPlayerMain(void)
{
    ...
    for (seqPlayer = (NNSSndSeqPlayer *)NNS_FndGetNextListObject(&sPrioList, NULL); seqPlayer != NULL; seqPlayer = next)
    {
        ...
        fader = SND_CalcDecibel(seqPlayer->initVolume) + SND_CalcDecibel(seqPlayer->extVolume) + SND_CalcDecibel(seqPlayer->player->volume)
