...
static NNSSndSeqData *LoadSeq(u32 fileId, NNSSndHeapHandle heap, BOOL bSetAddr)
{
    ...
    if (buffer == NULL)
    {
        buffer = NNSi_SndArcLoadFile(fileId, SeqDisposeCallback, bSetAddr ? (u32)NNS_SndArcGetCurrent() : 0, fileId, heap);
...
static SNDBankData *LoadBank(u32 fileId, NNSSndHeapHandle heap, BOOL bSetAddr)
{
    ...
    if (buffer == NULL)
    {
        buffer = NNSi_SndArcLoadFile(fileId, BankDisposeCallback, bSetAddr ? (u32)NNS_SndArcGetCurrent() : 0, fileId, heap);
...
static SNDWaveArc *LoadWaveArc(u32 fileId, NNSSndHeapHandle heap, BOOL bSetAddr)
{
    ...
    if (buffer == NULL)
    {
        buffer = NNSi_SndArcLoadFile(fileId, WaveArcDisposeCallback, bSetAddr ? (u32)NNS_SndArcGetCurrent() : 0, fileId, heap);
...
static SNDWaveArc *LoadWaveArcTable(u32 fileId, NNSSndHeapHandle heap, BOOL bSetAddr)
{
    ...
    if (waveArc == NULL)
    {
        ...
        waveArc = (SNDWaveArc *)NNS_SndHeapAlloc(heap, len + RESERVED_AREASIZE, WaveArcTableDisposeCallback, bSetAddr ? (u32)NNS_SndArcGetCurrent() : 0, fileId);
        if (waveArc == NULL)
        {
            return NULL;
        }

        readSize = NNS_SndArcReadFile(fileId, waveArc, (s32)(sizeof(waveArcHeader) + tableSize), 0);
...
static BOOL LoadSingleWave(SNDWaveArc *waveArc, int waveNo, u32 fileId, NNSSndHeapHandle heap)
{
    ...
    buffer = (SNDWaveData *)NNS_SndHeapAlloc(heap, len + RESERVED_AREASIZE, SingleWaveDisposeCallback, (u32)waveArc, (u32)waveNo);
    if (buffer == NULL)
        return FALSE;
