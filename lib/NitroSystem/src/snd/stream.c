...
BOOL NNS_SndStrmSetup(NNSSndStrm *stream, NNSSndStrmFormat format, void *buffer, u32 bufSize, int timer, int interval, NNSSndStrmCallback callback, void *arg)
{
    ...
    for (index = 0; index < stream->numChannels; index++)
    {
        ...
        SND_SetupChannelPcm(chNo, (SNDWaveFormat)format, chp->buffer, SND_CHANNEL_LOOP_REPEAT, 0, (int)(stream->chBufLen >> 2), 127, SND_CHANNEL_DATASHIFT_NONE, timer << 5, 64);
    }

    SND_SetupAlarm(stream->alarmNo, alarmTimer, alarmTimer, AlarmCallback, stream);
...
void NNS_SndStrmSetVolume(NNSSndStrm *stream, int volume)
{
    ...
    for (i = 0; i < stream->numChannels; i++)
    {
        ...
        SND_SetChannelVolume((u32)(1 << chNo), chVolume & 0xff, (SNDChannelDataShift)(chVolume >> 8));
    }
}
...
static void StrmCallback(NNSSndStrm *stream, NNSSndStrmCallbackStatus status)
{
    ...
    stream->callback(status, stream->numChannels, buffer, blockSize, stream->format, stream->callbackArg);
