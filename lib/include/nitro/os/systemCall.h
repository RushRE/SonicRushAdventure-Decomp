...
#define SVC_CpuClear(data, destp, size, bit)                                                                                                                                       \
    ...
    {                                                                                                                                                                              \
        ...
        SVC_CpuSet((u8 *)&(tmp), (u8 *)(destp), (MI_DMA_SRC_FIX | MI_DMA_##bit##BIT_BUS | ((size) / ((bit) / 8) & 0x1fffff)));                                                     \
    } while (0)

#define SVC_CpuClearArray(data, destp, bit) SVC_CpuClear(data, destp, sizeof(destp), bit)
#define SVC_CpuCopy(srcp, destp, size, bit) SVC_CpuSet((u8 *)(srcp), (u8 *)(destp), (MI_DMA_SRC_INC | MI_DMA_##bit##BIT_BUS | ((size) / ((bit) / 8) & 0x1fffff)))
#define SVC_CpuCopyArray(srcp, destp, bit) SVC_CpuCopy(srcp, destp, sizeof(srcp), bit)

#define SVC_CpuClearFast(data, destp, size)                                                                                                                                        \
    do                                                                                                                                                                             \
    {                                                                                                                                                                              \
        vu32 tmp = (vu32)(data);                                                                                                                                                   \
        SVC_CpuSetFast((u8 *)&(tmp), (u8 *)(destp), (MI_DMA_SRC_FIX | ((size) / (32 / 8) & 0x1fffff)));                                                                            \
    } while (0)

#define SVC_CpuClearArrayFast(data, destp) SVC_CpuClearFast(data, destp, sizeof(destp))
#define SVC_CpuCopyFast(srcp, destp, size) SVC_CpuSetFast((u8 *)(srcp), (u8 *)(destp), (MI_DMA_SRC_INC | ((size) / (32 / 8) & 0x1fffff)))
#define SVC_CpuCopyArrayFast(srcp, destp) SVC_CpuCopyFast(srcp, destp, sizeof(srcp))
