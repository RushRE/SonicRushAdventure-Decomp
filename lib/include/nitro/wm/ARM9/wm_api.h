#ifndef NITRO_WM_ARM9_WM_API_H
#define NITRO_WM_ARM9_WM_API_H
...
#define WM_SIZE_MP_PARENT_RECEIVE_BUFFER(childMaxSize, maxEntry, ksFlag)                                                                                                                               \
    (((sizeof(WMMpRecvHeader) - sizeof(WMMpRecvData)                                                                                                                                                   \
       + ((sizeof(WMMpRecvData) + (childMaxSize) + WM_HEADER_CHILD_MAX_SIZE - 2 + 2 + ((ksFlag) ? WM_SIZE_KS_CHILD_DATA + WM_SIZE_MP_CHILD_PADDING : 0)) * (maxEntry)) + 31)                           \
      & ~0x1f)                                                                                                                                                                                         \
     * 2)
...
#define WM_SIZE_MP_PARENT_SEND_BUFFER(parentMaxSize, ksFlag) (((parentMaxSize) + WM_HEADER_PARENT_MAX_SIZE + ((ksFlag) ? WM_SIZE_KS_PARENT_DATA + WM_SIZE_MP_PARENT_PADDING : 0) + 31) & ~0x1f)

#define WM_SIZE_MP_CHILD_SEND_BUFFER(childMaxSize, ksFlag) (((childMaxSize) + WM_HEADER_CHILD_MAX_SIZE + ((ksFlag) ? WM_SIZE_KS_CHILD_DATA + WM_SIZE_MP_CHILD_PADDING : 0) + 31) & ~0x1f)

#define WM_SIZE_DCF_CHILD_SEND_BUFFER(childMaxSize) (((childMaxSize) + WM_SIZE_MADATA_HEADER + 31) & ~0x1f)
...
WMErrCode WM_StartMPEx(WMCallbackFunc callback, u16 *recvBuf, u16 recvBufSize, u16 *sendBuf, u16 sendBufSize, u16 mpFreq, u16 defaultRetryCount, BOOL minPollBmpMode,
...
{
    return ((*idp1 == *idp2) && (*(idp1 + 1) == *(idp2 + 1)) && (*(idp1 + 2) == *(idp2 + 2)) && (*(idp1 + 3) == *(idp2 + 3)) && (*(idp1 + 4) == *(idp2 + 4)) && (*(idp1 + 5) == *(idp2 + 5)));
}

static inline BOOL WM_IsBssidEqual16(const u8 *idp1, const u8 *idp2)
{
    return ((*(u16 *)idp1 == *(u16 *)idp2) && (*(u16 *)(idp1 + 2) == *(u16 *)(idp2 + 2)) && (*(u16 *)(idp1 + 4) == *(u16 *)(idp2 + 4)));
}
...
{
    return (gameInfoLength >= WM_GAMEINFO_LENGTH_MIN && gameInfo->magicNumber == WM_GAMEINFO_MAGIC_NUMBER);
}
