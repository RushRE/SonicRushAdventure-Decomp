#ifndef NITROWIFI_SOC_STUB_H
#define NITROWIFI_SOC_STUB_H
...
int SOC_GetAddrInfo(const char *nodeName, const char *servName, const SOCAddrInfo *hints, SOCAddrInfo **res);
void SOC_FreeAddrInfo(SOCAddrInfo *head);
int SOC_GetNameInfo(const void *sa, char *node, unsigned nodeLen, char *service, unsigned serviceLen, int flags);
void SOC_GetMtu(SOCInterface *interface, s32 *mtu);

s32 ICMP_Cancel(ICMPInfo *info, s32 err);
s32 ICMP_Socket(int af, ICMPInfo *info);
s32 ICMP_Ping(ICMPInfo *info, const void *data, s32 len, const void *remote, ICMPCallback callback, s32 *result);
s32 ICMP_Close(ICMPInfo *info);

u16 IPi_CheckSum(IPHeader *ip);

#ifdef __cplusplus
}
#endif

#endif // NITROWIFI_SOC_STUB_H