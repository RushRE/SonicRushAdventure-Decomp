...
static ExTaskMain exBossAttackFuncTable[EXBOSS_ATTACK_COUNT] = {
    ...
    [EXBOSS_ATTACK_MAGMA_WAVE]                                = exBossSysAdminTask__Action_StartWave0,
...
void ExBoss_Main_Init(void)
{
    ...
    exDrawReqTask__SetConfigPriority(&work->aniBoss.config, 0xA800);
    exDrawReqTask__Func_2164218(&work->aniBoss.config);
...
void ExBossEarlyManager_Main_PickNextAttack(void)
{
    ...
    if (work->flags.isAttackReady)
    {
        ...
        {
            work->aniBoss.hitChecker.hitFlags.value_1 = TRUE;
            work->aniBoss.hitChecker.hitFlags.value_2 = TRUE;
    ...
    if (work->hurtInvulnDuration <= 0)
    {
        if (work->aniBoss.hitChecker.hitFlags.value_1 && work->aniBoss.hitChecker.hitFlags.value_2)
        {
            ...
            else
            {
                ...
                work->aniBoss.hitChecker.hitFlags.value_2 = FALSE;
                work->aniBoss.hitChecker.hitFlags.value_1 = FALSE;
            }
        }
    }
    else
