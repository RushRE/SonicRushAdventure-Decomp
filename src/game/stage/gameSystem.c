...
WirelessManagerRoom_Wireless storedRoomConnection_Wireless;
WirelessManagerRoom_DownloadPlay storedRoomConnection_DownloadPlay;
...
void GameSystem_Main_Early(void)
{
    ...
    if ((state->gameFlag & GAME_FLAG_IS_VS_BATTLE) != 0)
    {
        ...
        else if (WirelessManager__GetStatus() == WIRELESSMANAGER_STATUS_ERROR)
        {
            playerGameStatus.flags |= PLAYERGAMESTATUS_FLAG_NETWORK_ERROR;
        }
