...
static const u16 sUtfCharOffset[] = {
...
static const s32 sFontCharOffset[] = { 0,    0x5C, 0x5E, 0x61, 0x62, 0x63, 0x65,  0x69,  0x6A,  0x73,  0x77,  0x79,  0x7D,  0x81,  0x82,  0x8B,  0x8F,  0x91,  0x95,
...
static const s32 sFontCharPageSize[] = {
...
s32 GetFontCharacterFromUTF(u32 character)
{
    ...
    while (page < count)
    {
        ...
        if (sUtfCharOffset[nextID] <= character)
            page = nextID;
        else
            count = nextID - 1;
    }
    ...
    s32 fontChar = character - sUtfCharOffset[page];
    if (fontChar >= sFontCharPageSize[page])
        return 0;
    else
        return fontChar + sFontCharOffset[page];
}