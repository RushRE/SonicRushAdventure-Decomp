...
void CreateConnectionStatusIcon(ConnectionMode desiredConnectionMode, BOOL useEngineB, u8 paletteRow, u8 oamPriority, u8 oamOrder, s16 x, s16 y)
{
    ...
    if (sVars->task == NULL)
    {
        sVars->task = TaskCreate(ConnectionStatusIcon_Main, ConnectionStatusIcon_Destructor, TASK_FLAG_DISABLE_DESTROY, 0, TASK_PRIORITY_UPDATE_LIST_START + 0xEF00, TASK_SCOPE_0,
