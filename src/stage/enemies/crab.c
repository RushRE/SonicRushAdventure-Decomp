...
EnemyCrab *CreateCrab(MapObject *mapObject, fx32 x, fx32 y, fx32 type)
{
    ...
    Task *task = CreateStageTask(GameObject__Destructor, TASK_FLAG_NONE, 0, TASK_PRIORITY_UPDATE_LIST_START + 0x1500, TASK_GROUP(2), EnemyCrab);
    if (task == HeapNull)
        return NULL;
    ...
    ObjObjectAction2dBACLoad(&work->gameWork.objWork, &work->gameWork.animator, sSpriteList[0], GetObjectDataWork(OBJDATAWORK_10), gGameArchiveStage, OBJ_DATA_GFX_AUTO);
