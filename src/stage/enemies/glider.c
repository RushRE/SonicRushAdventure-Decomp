...
EnemyGlider *CreateGlider(MapObject *mapObject, fx32 x, fx32 y, fx32 type)
{
    ...
    Task *task = CreateStageTask(GameObject__Destructor, TASK_FLAG_NONE, 0, TASK_PRIORITY_UPDATE_LIST_START + 0x1500, TASK_GROUP(2), EnemyGlider);
    if (task == HeapNull)
        return NULL;
    ...
    ObjObjectAction2dBACLoad(&work->gameWork.objWork, &work->gameWork.animator, "/act/ac_ene_glider.bac", GetObjectFileWork(OBJDATAWORK_14), gGameArchiveStage, OBJ_DATA_GFX_AUTO);
...
void EnemyGlider_Action_Init(EnemyGlider *work)
{
    ...
    work->gameWork.objWork.displayFlag |= DISPLAY_FLAG_DISABLE_DRAW | DISPLAY_FLAG_DISABLE_LOOPING;
    ...
    work->gameWork.colliders[GAMEOBJECT_COLLIDER_SOLID].flag |= OBS_RECT_WORK_FLAG_NO_HIT_CHECKS;
...
void EnemyGlider_OnDefend(OBS_RECT_WORK *rect1, OBS_RECT_WORK *rect2)
{
    ...
    enemy->gameWork.objWork.displayFlag &= ~DISPLAY_FLAG_DISABLE_DRAW;
    ...
    enemy->gameWork.colliders[GAMEOBJECT_COLLIDER_SOLID].flag &= ~OBS_RECT_WORK_FLAG_NO_HIT_CHECKS;
