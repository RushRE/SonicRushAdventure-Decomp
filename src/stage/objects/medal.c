...
Medal *CreateMedal(MapObject *mapObject, fx32 x, fx32 y, fx32 type)
{
    ...
    Task *task = CreateStageTask(GameObject__Destructor, TASK_FLAG_NONE, 0, TASK_PRIORITY_UPDATE_LIST_START + 0x1800, TASK_GROUP(2), Medal);
    if (task == HeapNull)
        return NULL;
    ...
    ObjObjectAction2dBACLoad(&work->gameWork.objWork, &work->gameWork.animator, "/ac_itm_ring.bac", GetObjectFileWork(OBJDATAWORK_105), gGameArchiveCommon, OBJ_DATA_GFX_AUTO);
