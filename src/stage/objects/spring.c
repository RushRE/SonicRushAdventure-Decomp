...
Spring *CreateSpring(MapObject *mapObject, fx32 x, fx32 y, fx32 type)
{
    ...
    ObjObjectAction2dBACLoad(&work->gameWork.objWork, &work->gameWork.animator, "/act/ac_gmk_spring.bac", GetObjectFileWork(OBJDATAWORK_27), gGameArchiveStage, OBJ_DATA_GFX_AUTO);
    ObjActionAllocSpritePalette(&work->gameWork.objWork, SPRING_ANI_VERTICAL, 2);
    ...
    work->gameWork.colliders[GAMEOBJECT_COLLIDER_WEAK].onDefend = Spring_OnDefend;
...
void Spring_State_Activated(Spring *work)
{
    ...
    {
        work->gameWork.colliders[GAMEOBJECT_COLLIDER_WEAK].flag &= ~(OBS_RECT_WORK_FLAG_NO_ONATTACK_ONENTER | OBS_RECT_WORK_FLAG_SYS_HAD_ATK_THIS_FRAME | OBS_RECT_WORK_FLAG_SYS_WILL_DEF_THIS_FRAME);
        Spring_Action_Idle(work);
    }
}
