...
Tutorial *CreateTutorial(MapObject *mapObject, fx32 x, fx32 y, fx32 type)
{
    ...
    {
        ObjAction2dBACLoad(aniKey, "/act/ac_fix_key.bac", buttonSpriteSizes[i], &sVars.buttonPromptFile, gGameArchiveStage);
...
TutorialCheckpoint *CreateTutorialCheckpoint(MapObject *mapObject, fx32 x, fx32 y, fx32 type)
{
    ...
    ObjObjectAction2dBACLoad(&work->gameWork.objWork, &work->gameWork.animator, "/ac_gmk_check.bac", GetObjectFileWork(OBJDATAWORK_72), gGameArchiveCommon, OBJ_DATA_GFX_AUTO);
    ...
    ObjRect__SetOnDefend(&work->gameWork.colliders[GAMEOBJECT_COLLIDER_WEAK], TutorialCheckpoint_OnDefend);
...
void TutorialCheckpoint_OnDefend(OBS_RECT_WORK *rect1, OBS_RECT_WORK *rect2)
{
    ...
    checkpoint->gameWork.colliders[GAMEOBJECT_COLLIDER_WEAK].parent = NULL;
    checkpoint->gameWork.colliders[GAMEOBJECT_COLLIDER_WEAK].flag |= OBS_RECT_WORK_FLAG_NO_HIT_CHECKS;
